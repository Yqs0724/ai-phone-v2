...
// 默认人设：开箱即聊（用户随时可在「档案」里改掉）
const DEFAULT_PERSONA = `姓名：顾衍
27 岁，投行副总裁。表面毒舌挑剔，实则心思极细，记得住对方所有小习惯。
...
export function buildMessages() {
  ...
  // 第 3 层：剧情姿态（tone.js 每次对话后异步检测，这里读最新结果）
  const tone = get('tone')
  layers.push(`# 当前剧情姿态\n${TONE_MAP[tone] || TONE_MAP['热聊']}`)

  // 第 4 层：长期记忆（记忆服务负责提炼/衰减/取舍，这里只拿"当下最值得想起"的注入）
